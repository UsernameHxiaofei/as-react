...
class AddModal extends Component{
  ...
  addTeachers = () => { //确定事件
    ...
    let obj = {employees:this.props.addTeacher}
    this.props.SaveWorkLocationEmployee(obj, id)
  }
  ...
  render () {
    ...
    },
    {
      ...
      width:'140px',
      render: (record) => (
        <div>{record.empStatus == '0'? '在职':record.empStatus == '1'? '留职':'离职'}</div>
      ),
      key: '888',
    }, ]
...
const mapStateToProps = (state) => {
  const {Id,addTeacher, data,deptList,WZlist,addVisible,teacherList,totalNumber} = state.technicianStation
  return {Id,addTeacher, data,deptList,WZlist,addVisible,teacherList,totalNumber}
}

const mapDispatchToPros = (dispatch) => {
  const {SaveWorkLocationEmployee,SET_STATE,GetMdmDutyMstr,GetMdmOrgDepartmentMstr,GetTechnicianVo} = dispatch.technicianStation
  return {SaveWorkLocationEmployee,SET_STATE,GetMdmDutyMstr,GetMdmOrgDepartmentMstr,GetTechnicianVo}
}
